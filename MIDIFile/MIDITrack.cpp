...
//	elapsedTicks = _elapsedTimeTotal / (60000000L/(96 * bpm));  //original part of the library code.  This ultimately uses elapsed time and divides it by 
	...
	/* if (mf->firstdelta == true){
	mf->setTrackFirstDelta(deltaT);
	mf->firstdelta = false;
	mf->backupcount = tickCount;
	
	} */	
	_total_deltas = ( old_delta +  deltaT  )  ; // add the old delta time to the new one.
	//need to code a way to keep a running total of the deltas	?
	
	old_delta = (_total_deltas  -  deltaT)  ;
