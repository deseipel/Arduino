...
//	unsigned long     _delta; 
	...
	long long   _elapsedTicks;    // the total ticks elapsed for the track
	long long   _total_deltas;       //the new total delta for the track
...
class MIDIFile 
{
  public:
	...
	void    setDelta(uint32_t delta);
	void 	resetDelta(uint32_t ddelta);
	uint32_t getDelta(void);
	long long 	getTickCount();              //passes in the current count of ticks that have elapsed.
	void    setTickCount(long long Tcount);
	uint32_t getTrackFirstDelta(void);  
	void 	 setTrackFirstDelta(uint32_t tfd);  
	void 	 setDcount(uint32_t dcnt);
	uint32_t getDcount(void);   // a counter for debugging.
    ...
    const char* getFilename(void);
		byte 		getTrackNo(void);       //gets the current trackid.
  ...
  protected:
	...
	unsigned long 	total_track_deltas;   // per track deltas
	uint32_t _tfd;  //total track deltas
	bool firstdelta;  //
	long long backupcount;  
	long long dcount;  // used to count how many tracks the code is cycling through.
