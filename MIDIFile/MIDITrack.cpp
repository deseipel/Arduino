...
// is there anything to process?
	...
	total_deltas = ( old_delta +  deltaT  )  ; // add the old delta time to the new one.
	//need to code a way to keep a running total of the deltas	?
	
	old_delta = (total_deltas-deltaT)  ;
	
	//  if (elapsedTicks < deltaT)  //part of the original code, comparing elapsed ticks and the read delta.
	//	return(false);  		
	
	if ((tickCount < total_deltas) || (tickCount < 0))  //
	//if(tickCount < deltaT)
		{
		total_deltas=(old_delta);
	...
	//mf->setTickCount(0);   //used when I wasn't using a running total of ticks/deltas	
